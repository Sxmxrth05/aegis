...
- **Windows "ghost" listener on `127.0.0.1:<port>` after a restart:** occasionally a prior backend process leaves a listening socket on `127.0.0.1:<port>` (e.g. `8000`) that keeps serving its OLD in-memory code to `localhost`/`127.0.0.1` requests — including `curl` health checks — even after you kill and restart the server. The telltale symptom: your fix behaves correctly when you exercise the code directly (a one-off script) but the live server/browser still shows the old, unfixed behavior. Unlike the stale-PID case above, this one doesn't even resolve to a real PID: `Get-NetTCPConnection -LocalPort <port>` reports an `OwningProcess`, but `Get-Process`, `Get-CimInstance Win32_Process`, and `taskkill /F /PID` all fail to find it — it's an orphaned OS-level socket, not a process you can kill. Confirm with `netstat -ano | findstr :<port>` — if you see two LISTENING rows, one on `0.0.0.0:<port>` (your fresh process) and one on `127.0.0.1:<port>` (the ghost, on a different PID), that's this issue. Workarounds: bind your dev server to `0.0.0.0` explicitly (not just the default, which sometimes still resolves to the specific-address form) so the fresh process's binding isn't shadowed by the ghost's more-specific `127.0.0.1` entry for other clients on the network; for verifying `127.0.0.1`/`localhost` traffic specifically, temporarily run on a different port and point the client at that instead; a full reboot reliably clears it (closing the browser tab/process holding the other end of the ghost's connection may also release it, but isn't guaranteed).
