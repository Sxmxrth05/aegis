...
| B3 | `orchestrator/orchestrator.py` (Monitor slice) — wraps Dev A's output as `ConjunctionAlert` events, broadcasts | `[M]` | Merged to `main` via `dev-b/monitor-broadcast` (commit `74cd70c`, rebased onto Dev A's PR #1). Originally ran on a hardcoded fixture pending Dev A's A3; since A3 landed, `Orchestrator.detect_conjunctions()` calls the real `monitor_agent.detect_conjunctions()` (confirmed live in the pre-Checkpoint-1 audit — real alert values, not the old fixture's). Wired to `/ws/monitor` in `main.py`: connect → snapshot → broadcast. Verified manually with a Python `websockets` client: snapshot arrives first at `sequence: 1`, alert follows at `sequence: 2` with the correct envelope shape. **Closed the mock-data gap open since D4:** `/ws/monitor`'s snapshot payload was `{"conjunctions": []}` — no tracked-object data at all, which is why the frontend has been falling back to `mockTrackedObjects.ts` this whole time. Added `Orchestrator.monitor_snapshot_payload()` (backed by a new `load_tracked_objects_fixture()` helper) which loads Dev A's real `data/fixtures/tracked_objects.json`, validates each entry against the canonical Pydantic `TrackedObject`, and includes it in the snapshot as `trackedObjects` (camelCase — matches the frontend store's `SnapshotPayload` type exactly, since this is a raw dict sent as-is, not through Pydantic field aliasing). `main.py`'s TODO comment about this is now removed. **No frontend code changes were needed** — `Monitor.tsx`/`Negotiate.tsx` already read `trackedObjects` from `useNegotiationStore` with `mockTrackedObjects.ts` only as a fallback (built that way from D4/D5/D6 onward specifically so this would self-correct); confirmed live that both screens' legends switched from "mock data (live tracking not yet available)" to "live WebSocket feed" with zero code edits. Re-verified the full kill/restart cycle from D5's original test: killed the real backend mid-session — `Reconnecting` status, legend correctly fell back to mock data, globe kept showing the 5 mock satellites; restarted the backend — recovered to `Live` with the legend switching back to the real fixture data. All screenshotted. |
...
**2026-09-13 Dev C — globe background satellite layer (branch `dev-c/globe-background-satellites`, NOT yet merged):** Added a decorative background satellite cloud to the Monitor globe — cosmetic visual density only, zero impact on detection logic, Tracking Register counts, or HUD labels. Three new/modified files: (1) `frontend/src/components/globe/backgroundSatellites.ts` [NEW] — fetches 20 real objects from CelesTrak's `visual` group (intentionally a different group from `LOCKED_OBJECTS`'s `stations` group; `scenario.py` and `celestrak.py` not touched), propagates each with `satellite.js` SGP4, falls back silently to a hardcoded set of 20 geo-distributed static positions if the fetch fails. (2) `Globe.tsx` [MODIFIED] — additive-only: new optional `backgroundObjects?: BackgroundPoint[]` prop; accurate ThreeGlobe spherical projection mapping; a `useEffect` that builds a Three.js `Points` mesh named `aegis-bg-satellites` added directly to `globe.scene()` (completely outside `react-globe.gl`'s `pointsData`/HUD/label pipeline — no label-overlap risk, no impact on `detect_conjunctions()`); cleanup extended to dispose the mesh on unmount. Dev D's `forwardRef`/`GlobeHandle`/transition code is untouched — this is purely additive. (3) `GlobeStage.tsx` [MODIFIED] — one-shot `useEffect` on mount calls `fetchBackgroundSatellites()` and stores result in local state; passes it to `<Globe backgroundObjects={...} />`. `Monitor.tsx` untouched. Background objects rendered at `#6d94c7`, `opacity: 0.68`, `size: 4.8px` non-attenuating — visually distinct from active-blue (`#60a5fa`) monitored points. `tsc --noEmit` exit code 0 (zero new errors). Ready for review against `main` before merge.
...
**2026-09-12 Dev D redesign update:** Landing and Monitor were rebuilt around a dense, rectilinear operations-console system (IBM Plex typography, square telemetry grids, semantic status lights, restrained scan motion). Monitor retains the single existing Globe and all live/fallback data behavior, replacing the rounded floating legend with tracking-register, conjunction-channel, and frame-telemetry overlays. Typecheck/lint pass; headless Edge visual QA passed at 1440×1200.

**2026-09-12 Globe consolidation + label-overlap fix (Dev B, covering Dev D, branches `dev-d/fix-overlapping-globe-labels` then `dev-d/consolidate-landing-globe`):** A same-day landing-page redesign had introduced `LandingGlobe.tsx`, a ~180-line near-duplicate of `Globe.tsx` (own copy of the atmosphere/starfield/lighting setup, its own tiny 8-point synthetic dataset) rather than reusing the existing component — exactly the kind of parallel-implementation drift `ui-registry.md` exists to prevent. Consolidated: deleted `LandingGlobe.tsx` entirely; `Landing.tsx` now renders `<Globe mode="landing" trackedObjects={...} conjunctionAlert={...} />`, sourcing real data from `useNegotiationStore` (same pattern, same `mockTrackedObjects.ts` fallback, as `Monitor.tsx`) instead of hardcoded sample points. Added `'landing'` to `Globe.tsx`'s `mode` union — ambient/non-interactive framing (auto-rotate, no zoom/pan, fixed initial camera) layered on the exact same rendering pipeline every other mode uses, per `ui-registry.md`'s Globe Component entry. Along the way, fixed a real overlapping-label bug this consolidation was meant to surface: docked/co-located objects (ISS complex, CSS cluster) propagate to identical screen positions, so per-object HUD labels stacked illegibly on both Landing and Monitor. Fixed with `Globe.tsx`'s new `clusterHudPoints()` (three passes — named docked groups via new `globe/dockedGroups.ts`, generic proximity clustering, and a final de-collision pass for two *different* clusters that happen to project close together at a given camera angle) rendering one combined `"<LABEL> (<count>)"` marker with a hover tooltip listing individual members. Verified live via a single real SPA session (so Landing and Monitor share the same WebSocket connection/store state, not two independent reconnects): both pages show byte-identical `"ISS COMPLEX (6)"`/`"CSS CLUSTER (5)"` labels, no overlapping/garbled text anywhere on the globe. `tsc --noEmit` clean on both fixes; `npm run build`'s pre-existing `@types/three` gap is unrelated and untouched (confirmed present on unmodified `main` too). **Noted but deliberately not built:** this consolidation is a stated prerequisite for a planned landing→Monitor transition animation — that only makes sense once "the globe" is genuinely one component instance/logic rather than two look-alike implementations. The transition itself is out of scope for this task.
...
**2026-09-12 full four-screen sanity audit (Dev B, `dev-b/fix-docked-conjunctions-and-test-data`) — two real bugs found and fixed:**
- **`residual_risk` unit bug:** `validation_agent.py` was assigning a raw miss-distance (km) into `residual_risk`, a field the frontend renders as a percentage (`(residual_risk * 100).toExponential(2)%`) — e.g. showing `6.80e+2%` for a 6.8km miss. Root cause was a foundational gap: neither `schemas/negotiation.py`'s `Resolution.residual_risk` nor `architecture.md`'s DB spec ever documented the field's intended units. Fixed by adding `residual_risk_from_distance()` (a documented Gaussian-falloff heuristic treating `CONJUNCTION_THRESHOLD_KM` as a 3-sigma boundary — this codebase has no position-covariance data for a real Pc/Foster's-method calculation) and applying it at both real distance→risk conversion points in `validation_agent.py`, plus the equivalent fast path in `negotiator.py`. Verified live: `4.01e-3%` for a 7.5km miss, matching direct calculation. Not yet merged to `main`.
- **Docked-object false conjunctions:** `Orchestrator.detect_conjunctions()` (Phase 2's move to the full 20-object curated set) was flagging physically docked/co-located spacecraft — the ISS complex (25544/36086/49044/67796/68689/68837) and CSS complex (48274/53239/54216/69049/69180), all propagating to identical positions — as 0.000km/0.000km/s "conjunctions," generating full negotiation sessions for a non-problem. Fixed with an explicit `DOCKED_OBJECT_GROUPS` config in `orchestrator.py` (cross-referenced to `celestrak.py`'s `LOCKED_OBJECTS`, which defines the curated set) that excludes same-group pairs before returning alerts. Verified live: all 25 raw docked-pair alerts filtered to zero, correctly falling back to the demo hardcoded alert. Committed (`a1ae926`) on `dev-b/fix-docked-conjunctions-and-test-data`.
- Also removed a synthetic `sanity-check-no-safe-maneuver-0001` test record that had leaked into the real `aegis.db` during this audit (DB isn't git-tracked, so nothing to commit — just a runtime data fix).
- **Flagged, not fixed (outside this task's scope — recent, deliberate work by other devs):** `/api/trajectory/{conjunction_id}` ignores its own path param and always returns the same hardcoded default scenario regardless of which session is being inspected; History's "Inspect" links navigate to `/negotiate/:id` but `Negotiate.tsx` ignores the URL param and always shows the current live alert instead of that record's actual historical data.
...
| Dev D | About page (mission framing, data sources, labeled roadmap of unbuilt Phase 2+ items); final visual QA vs. `ui-tokens.md`/`ui-rules.md` (no full colored-background cards) | `[x]` — Replaced placeholder with system manifest, safety invariants, dependency provenance, honest simulation scope, and current roadmap; aligned Landing/Monitor/Negotiate/History/About to the same dense operations-console language. No full colored-background cards. |
...
| 2026-09-12 | Dev B (covering Dev C) | Defined `residual_risk` as a Gaussian-falloff function of miss distance (`residual_risk_from_distance()` in `validation_agent.py`), rather than the raw distance value it was previously (mis)assigned. | The field's name and the frontend's percentage formatting both imply a probability, but neither `schemas/negotiation.py` nor `architecture.md` ever documented the intended units — this was a foundational ambiguity, not a one-line typo. No covariance/uncertainty data exists in this codebase for a real Pc (Foster's method) calculation, so a clearly-labeled heuristic was used instead of fabricating precision the data doesn't support. | Every real call site producing a final `Resolution.residual_risk` (both `validation_agent.py` and `negotiator.py`'s `APPROVED_NO_ACTION` fast path) now derives it from this one function — future changes to the risk model only need to happen in one place. |
| 2026-09-12 | Dev B | Excluded physically docked/co-located NORAD ID pairs from `Orchestrator.detect_conjunctions()` via an explicit `DOCKED_OBJECT_GROUPS` list, rather than changing `monitor_agent.py`'s pairwise detection math itself. | The false positives are a data-modeling artifact of the 20-object curated set (docked modules propagate to identical positions), not a flaw in the distance/threshold logic itself, which is correct and unowned by Dev B. Filtering at the orchestrator layer (Dev B's own file) avoided touching Dev A's `monitor_agent.py`. | `DOCKED_OBJECT_GROUPS` must be kept in sync with `celestrak.py`'s `LOCKED_OBJECTS` if the curated 20-object set ever changes — cross-referenced in both files. |
