...
Built in `frontend/src/components/globe/Globe.tsx` — one `react-globe.gl` instance, driven entirely by props (never rebuilt per screen, per architecture.md invariant 9). This is the ONLY globe implementation in the app — `LandingGlobe.tsx` (a near-duplicate, briefly introduced for the hero section) was consolidated into this component and deleted on 2026-09-12; Landing.tsx now renders `<Globe mode="landing" ... />` like every other screen, so the label-clustering fix below and any future globe work only need to happen in one place.
- `trackedObjects: TrackedObject[]` (type in `globe/types.ts`, mirrors `schemas/tracked_object.py` field-for-field), `mode: 'live' | 'conjunction' | 'trajectory' | 'landing'`, optional `conjunctionAlert` (mirrors `schemas/conjunction.py`).
